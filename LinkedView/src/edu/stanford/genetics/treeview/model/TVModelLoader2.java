...
public class TVModelLoader2 implements ProgressTrackable {
	 ...
	 * @param strings
	 */
	private final String[] phases = new String[] { "Loading Cdt",
	...
	public TVModelLoader2(final TVModel targetModel) {

		this(targetModel, targetModel.getFrame());
	}

	public TVModelLoader2(final TVModel targetModel, final Frame parent) {
	...
	public void setException(final LoadException exception) {
	...
	public void setHadProblem(final boolean hadProblem) {
	...
	public class DummyLoadProgress implements LoadProgress2I {
		...
		@Override
		public void setPhaseValue(final int i) {

			this.phaseValue = i;
		}

		@Override
		public void setPhases(final String[] strings) {
		}
	...
	protected void setPhase(final int i) {
	...
	protected void run() {
		...
		try {
			...
			final JLabel l1 = new JLabel("Out of memory, allocate more RAM");
			l1.setForeground(GUIParams.TEXT);
			l1.setFont(fontS);
			temp.add(l1, "pushx, wrap");

			final JLabel l2 = new JLabel(
					"see Chapter 3 of Help->Documentation... "
							+ "for Out of Memory");
	...
	/**
	 * This routine expects a vector of strings It calls various routines that
	 * parse the expression data and annotations.
	 * 
	 ...
	 * 
	 * Uses new style code that takes advantage of RectData structure and is
	 * fast.
	 */
	protected void findCdtDimensions(final RectData tempVector) {
		println("Finding Cdt Dimensions");

		// // String [] firstLine =(String []) tempVector.elementAt(0);
		int gweightCol = -1;
		final int rectCol = tempVector.getCol();
		...
		if (tempVector.getColumnName(0).equalsIgnoreCase("EWEIGHT")) {
			...
			final int rectRow = tempVector.getRow();
			for (int i = 0; i < rectRow; i++) {
				// // String s = ((String []) tempVector.elementAt(i))[0];
				final String s = tempVector.getString(i, 0);
	 ...
	 * 
	 * @param tempVector
	 *            RectData contain annotation info
	 */
	protected void loadArrayAnnotation(final RectData tempVector) {
		...
		for (int i = 0; i < nExprPrefix; i++) {
			final String[] tokens = (String[]) tempVector.elementAt(i);
	 ...
	 *            RectData contain annotation info
	 */
	protected void loadGeneAnnotation(final RectData tempVector) {
		...
		final String[] firstLine = (String[]) tempVector.elementAt(0);
	...
	protected void loadCdtData(final RectData tempVector) {
		...
		for (int gene = 0; gene < nGene; gene++) {
			...
			setValue(gene);
			final String[] tokens = (String[]) tempVector.elementAt(gene
					+ nExprPrefix);
			int found = tokens.length - nGenePrefix;
			if (found != nExpr) {
				setHadProblem(true);
				String err = "Wrong number of fields for gene " + tokens[0]
						+ " row " + (gene + nExprPrefix) + " Expected " + nExpr
						+ ", found " + found;
				println(err);
				err = "Line contains:";
				err += " " + tokens[0];
			...
			for (int expr = 0; expr < found; expr++) {
				try {
					exprData[gene * nExpr + expr] = makeDouble(tokens[expr
							+ nGenePrefix]);
				} catch (final Exception e) {
					setHadProblem(true);
					println(e.getMessage());
					println("Treating value as not found for gene " + gene
							+ " experiment " + expr);
	...
	protected double makeDouble(final String s) throws NumberFormatException {
		if (s == null) {
			...
			try {
				...
				final double retval = tmp.doubleValue();
	...
	protected int makeInteger(final String s) {
		if (s == null) {
			setHadProblem(true);
			...
			try {
				final Integer tmp = new Integer(s);
				final int retval = tmp.intValue();
				return retval;
			} catch (final Exception e) {
				setHadProblem(true);
				println("returning -1 for badly formatted int '" + s + "'");
	...
	private void parseATR(final RectData tempVector) throws LoadException {
		final String[] firstRow = (String[]) tempVector.firstElement();
		if ( // decide if this is not an extended file..
		(firstRow.length == 4)// is the length classic?
				&& (firstRow[0].equalsIgnoreCase("NODEID") == false) // does it
																		// begin
																		// with
																		// a
																		// non-canonical
			...
			targetModel.setAtrPrefix(new String[] { "NODEID", "LEFT", "RIGHT",
					"CORRELATION" });
			final String[][] atrHeaders = new String[tempVector.size()][];
			...
			final String[][] atrHeaders = new String[tempVector.size() - 1][];
			for (int i = 0; i < atrHeaders.length; i++) {
	...
	private void parseGTR(final RectData tempVector) throws LoadException {
		final String[] firstRow = (String[]) tempVector.firstElement();
		if ( // decide if this is not an extended file..
		(firstRow.length == 4)// is the length classic?
				&& (firstRow[0].equalsIgnoreCase("NODEID") == false) // does it
			...
			targetModel.setGtrPrefix(new String[] { "NODEID", "LEFT", "RIGHT",
					"CORRELATION" });
			final String[][] gtrHeaders = new String[tempVector.size()][];
			...
			final String[][] gtrHeaders = new String[tempVector.size() - 1][];
			for (int i = 0; i < gtrHeaders.length; i++) {
		...
		/*
		 * / test out the GtrHeaders... HeaderInfo gtrHeaders =
		 * targetModel.getGtrHeaderInfo(); for (int i = 0; i <
		 * gtrHeaders.getNumHeaders(); i++) { String nodeId =
		 * gtrHeaders.getHeader(i, "NODEID"); String leftId =
		 * gtrHeaders.getHeader(i, "LEFT"); String rightId =
		 * gtrHeaders.getHeader(i, "RIGHT"); String corr =
		 * gtrHeaders.getHeader(i, "CORRELATION"); System.out.println("node " +
		 * nodeId + " has left ID " + leftId + ", right id " + rightId +
		 * "corre " + corr); }
		 */
	}
	 ...
	 * setLength(source.size()); int found = 0; for (int row = 0; row <
	 * source.size(); row++) { if (getFinished()) break; setValue(row); String
	 * [] tempString = new String[3]; String [] tokens = (String [])
	 * source.elementAt(row); for (int i = 0; i < 3; i++) { tempString[i] =
	 * tokens[i]; } nvec.addElement(tempString); nvec.addElement(new
	 * Double(tokens[3])); found++; } return found; }
	 * 
	 * private void setValue(int j) { final int i = j; Runnable update = new
	 * Runnable() { public void run() { loadProgress.setValue(i); } };
	 * SwingUtilities.invokeLater(update); }
	 */
	protected void println(final String k) {
		final String s = k;
		// LogPanel.println(s);
	...
	public static void main(final String[] argv) {
		...
		final JButton button = new JButton("load " + argv[0]);
		button.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent e) {
				model.setSource(fileSet);
				final TVModelLoader2 loader = new TVModelLoader2(model, frame);
				try {
					loader.loadInto();
	...
	protected int nGene;
	/**
	 * these internal variables are used to keep track of the state of the
	 * tvmodel as it is being loaded.
	 */
	protected int nExpr;
