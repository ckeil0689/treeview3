...
/**
 * Static class (java way...) to be used for loading read-only 
 * preview data for the user. The information can then be used to specify 
 * parameters and option for the real loading process.
 * @author chris0689
 *
 */
public final class PreviewLoader {
	...
	/* 
	 * For recognizing common labels in a file.
	 * Matches (non case-sensitive):
	 * YORF, ORF, EWEIGHT, GWEIGHT, WEIGHT, COMPLEX, NAME, GID, UID, AID, ID 
	 */
	private final static String COMMON_LABELS = "(?i)(COMPLEX|NAME|^.*ORF$"
			+ "|^.*ID$|^.*WEIGHT$)";
	
	/* 
	 * For recognizing supposed numeric data in a file.
	 * Matches (non case-sensitive):
	 * NA, N/A, NAN, NONE, EMPTY, MISS, MISSING
	 */
		...
		try {
			...
			while ((line = br.readLine()) != null && count < LIMIT) {
				...
				for (int i = 0; i < lineAsStrings.length; i++) {
					 ...
					 * Data found if:
					 * 1) Is numeric
					 * 2) OR found a N/A (or equivalent) symbol)
					 * 3) current string index is bigger than last known
					 * column header index
					 * 4) Current line does NOT have a common known header 
					 * (e.g. EWEIGHT) in the line, which may be a label but 
