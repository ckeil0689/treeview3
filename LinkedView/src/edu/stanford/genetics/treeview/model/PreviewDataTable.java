...
//		setModel(getNewDefaultTableModel());
	 ...
	 * @param maxRow The number of rows included as labels.
	 * @param maxCol The number of columns included as labels.
	 */
	 ...
	 * part of a label row or column. 
	 * The Lists of integer values are supposed to be updated by the JSpinners
	 * for including rows/ columns as labels. 
	 * The values in these lists are then compared with specific cell indices
	 * and if there's a match, the cells will be colored.
	 * @author chris0689
	 *
	 */
...
//			label_cols.add(Integer.valueOf(0));
		...
		/**
		 * TODO divide by row/ column
		 * Specifies how many rows/ columns are considered to be labels.
		 * Starts at index 0 at all times because labels in a dataset should
		 * always be the very first rows/ columns.
		 * Should be called whenever JSpinners are updated in 
		 * DataImportController.
		 * @param maxRow The number of continuous rows considered to be labels.
		 * @param maxCol The number of continuous columns considered to be labels.
		 */
		public void includeLabelsUpTo(int maxRow, int maxCol) {
