...
/**
 *  Defines an interface for storage of key-value pairs. 
 *  Essentially all the configuration information for Java TreeView 
 *  is stored using this interface. You will encounter two 
 *  implementing classes. The first, most common one is an 
 *  inner class of XmlConfig, which simply presents an interface 
 *  to edit an xml document. Thus, when you mess with that 
 *  inner class through this interface, you're actually writing XML. 
 *  The second is the DummyConfigNode, which you can use for 
 *  prototyping stuff or if you just want to use this interface 
 *  to store key-value pairs in a non-persistent fashion.
 * 
 * The easiest way to make an object persistent across different 
 * runs of the program is to bind it to a ConfigNode returned by 
 * XmlConfig (which is bound to a file on disk), and then just store 
 * all state information in the ConfigNode. Whenever the XmlConfig is saved, 
 * it will automatically save the state of your object. 
 * just make sure you save it before you exit!
 *
