...
public abstract class TreeViewApp implements WindowListener {
	...
	protected static String announcementUrl = 
	...
	private UrlPresets geneUrlPresets;
	private UrlPresets arrayUrlPresets;
	...
	*/
	public TreeViewApp(XmlConfig xmlConfig, boolean isApplet) {
		...
		geneUrlPresets = new UrlPresets(getGlobalConfig()
				.getNode("GeneUrlPresets"));
		arrayUrlPresets = new UrlPresets();
		arrayUrlPresets.bindConfig(getGlobalConfig()
				.getNode("ArrayUrlPresets"));
	...
	/** 
	* Getter for updateUrl, a string representing a website 
	* where you can download newer versions.
	*/
	...
	/**
	 *  creates a new ViewFrame window displaying 
	 *  the specified <code>FileSet</code>
	 *
	 * @param  fileSet            FileSet to be displayed
	 * @exception  LoadException  If the fileset cannot be loaded, 
	 * the window is closed and the exception rethrown.
	 * @throws LoadException 
	 */
	...
	*/
	...
	/**
	 *  A WindowListener, which allows other windows to react 
	 *  to another window being opened.
	 *
	 * @param  e  A window opening event. Used to add the 
	 * window to the windows list.
	 */
	@Override
	...
	/**
	 *  rebuilds all the window menus. Should be called whenever 
	 *  a <code>ViewFrame</code> is created, destroyed, or changes its name. 
	 *  The first two cases are handled by <code>TreeViewApp</code>, 
	 *  the <code>ViewFrame</code> itself should call this method when 
	 *  it changes its name.
	 */
...
//			rebuildWindowMenu( source.getWindowMenu());
	...
	public void closeAllWindows() {
		...
		Enumeration<Window> e  = windows.elements();
