...
public class XmlConfig {
     ...
     * Construct new configuration information source
     * 
     * @param xmlUrl url from which the text came from, since 
     * nanoxml sucks so damn hard.
     */
    ...
    private class XmlConfigNode implements ConfigNode{
		...
		public double getAttribute(String string, double d) {
			...
			Double val = Double.valueOf(root.getAttribute(string, 
					Double.toString(d)));
     ...
     * 
     * Wherenever a settings panel which affects the config is closed, 
     * we want those changes to be saved.
	 *
	 * returns a WindowListener which will store theconfig every time 
	 * a window it listens on is closed.
	 * 
