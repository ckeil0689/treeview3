...
public class PluginManager {
	...
	public void loadPlugins(File[] f_jars, boolean showPopup) {
		...
		for (int i = 0; i < f_jars.length; i++) {
			...
			try {
				URL jarURL = new URL("jar:"+f_jars[i].toURI().toURL().toString()+"!/");
	...
	public PluginFactory getPluginFactoryByName(String name) {
		Enumeration e = pluginFactories.elements();
		while (e.hasMoreElements()){
			PluginFactory factory = (PluginFactory) e.nextElement();
