...
import edu.stanford.genetics.treeview.ConfigColorSetEditor;
import edu.stanford.genetics.treeview.DummyConfigNode;
import edu.stanford.genetics.treeview.SettingsPanel;

/**
 * This class allows graphical editing of ScatterColorPresets Also included is a
 * class to pop up a configuration window.
 */
public class ScatterColorPresetEditor extends JPanel implements SettingsPanel {
	private final ScatterColorPresets presets;
	 ...
	 */
	public ScatterColorPresetEditor(final ScatterColorPresets up) {
		super();
		presets = up;
	...
	public static void main(final String[] argv) {
		final ScatterColorPresets p = new ScatterColorPresets(
				new DummyConfigNode("ColorPresets"));
		final ScatterColorPresetEditor e = new ScatterColorPresetEditor(p);
	...
	private class PresetEditPanel extends JPanel {
		...
		public void redoLayout() {
			...
			addS.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(final ActionEvent e) {
					final ScatterColorSet[] toAdd = ScatterColorPresets.defaultColorSets;
		...
		private void removePreset(final int mark) {
			final ScatterColorSet[] tPresetColors = new ScatterColorSet[presetColors.length - 1];
		...
		private void createPreset() {
			final ScatterColorSet toAdd = new ScatterColorSet();
			addPreset(toAdd);
		}

		private void addPreset(final ScatterColorSet toAdd) {
			final ScatterColorSet[] tPresetColors = new ScatterColorSet[presetColors.length + 1];
			final JTextField[] tPresetNames = new JTextField[presetNames.length + 1];
			final JRadioButton[] tDefaultButtons = new JRadioButton[defaultButtons.length + 1];
