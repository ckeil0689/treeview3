...
public class LeftTreeDrawer extends TreeDrawer {
	...
	/**
		 ...
		 * @param xScaleEq The equation to be applied to scale the index of 
		 * the nodes to graphics object.
		 * @param yScaleEq The equation to be applied to scale the correlation 
		 * of the nodes to the graphics object.
			...
			while (remaining.empty() == false) {
				...
				if (node == selected) {
					if (isSelected == false) {
						...
						// isSelected is true, so we're pulling the selected node 
						// off the second time.
						isSelected = false;
						continue;
					}
