...
//		ymap.notifyObservers();
	...
	private static void updateSelection(TreeSelectionI origin,
		...
		if (adjusting.getNSelectedIndexes() == 0) {
			 ...
			 * because in the eventlistener you can only deselect indices for
			 * the local tree selection. other axis here!
			 */
	...
	public void mouseWheelMoved(final MouseWheelEvent e) {
		...
		if (e.isAltDown()) {
			if (notches < 0) {
				// This ensures we only zoom toward the cursor when the cursor
				// is
				// over the map
				if (status != null) {
	...
	public void smoothZoomTowardPixel(int xPxPos, int yPxPos) {
		...
		// This will tell us how out of sync the aspect ratio has gotten from
		// the
		...
		// for equal numbers of visible cells on both axes -possibly
		// unnecessary)
		if ((numXCellsShouldHave % 1) == 0
			...
			// numXCellsShouldHave = aspectRatio*numYCells //If we decide to
			// make Y "correct", we can calculate what X should be
			// numXCellsShouldHave/numYCells =
			// numXCells/(numYCells+_numYCellsShouldHave_) = aspectRatio //We do
			// not know _numYCellsShouldHave_, solving for it, we have:
			// _numYCellsShouldHave_ =
			// numXCells*numYCells/numXCellsShouldHave-numYCells //Then,
			// canceling out numYCells, we have:
			// _numYCellsShouldHave_ = numXCells/numXCellsShouldHave-1
			// //_numYCellsShouldHave_ will be a ratio, i.e. the same as
			// targetZoomFracCorrection
			targetZoomFracCorrection = Math.abs(numXCells / numXCellsShouldHave
					- 1);
			zoomXVal = xmap.getBestZoomInVal(xPxPos, targetZoomFrac);
			zoomYVal = ymap.getBestZoomInVal(yPxPos, targetZoomFrac
					+ targetZoomFracCorrection);
	...
	public void smoothZoomFromPixel(int xPxPos, int yPxPos) {
		...
		if ((numXCellsShouldHave % 1) == 0
			...
			// If no zoom has occurred
			if (zoomXVal == 0 && zoomYVal == 0) {
				// If there's room to zoom in both dimensions, try to zoom out
				// 2,
			...
			// If the X axis should have more cells on it, zoom out on the x
			// axis
			// more
			zoomXVal = xmap.getBestZoomOutVal(xPxPos, targetZoomFrac
					+ targetZoomFracCorrection);
			...
			// Else, the X axis should have fewer cells on it, so zoom out on
			// the
			// y axis more.
			// The fraction calculation is different for the Y axis. The
			// following calculation is based on the merging of a few equations:
	...
	public void smoothIncrementalZoomOut() {
		 ...
		 */
		smoothZoomTowardSelection(0, (xmap.getMaxIndex() + 1), 0,
				(ymap.getMaxIndex() + 1));
	...
	public void smoothAnimatedZoomOut() {
		smoothAnimatedZoomTowardSelection(0, (xmap.getMaxIndex() + 1), 0,
				(ymap.getMaxIndex() + 1));
	}
	...
	public void smoothAnimatedZoomTowardSelection(int selecXStartIndex,
			int numXSelectedIndexes, int selecYStartIndex,
		...
		while (xmap.getNumVisible() != numXSelectedIndexes
			...
			int[] redrawDims = smoothZoomTowardSelection(selecXStartIndex,
					numXSelectedIndexes, selecYStartIndex, numYSelectedIndexes);
		...
		// level or scroll position, so we should do a zoom & scroll here just
		// to
		...
		if (xmap.getNumVisible() != numXSelectedIndexes) {
			...
			xmap.zoomToSelected(selecXStartIndex, (selecXStartIndex
					+ numXSelectedIndexes - 1));
		}
		if (ymap.getNumVisible() != numYSelectedIndexes) {
			// LogBuffer.println("Adjusting final Y zoom");
			ymap.zoomToSelected(selecYStartIndex, (selecYStartIndex
					+ numYSelectedIndexes - 1));
		}
	...
	public int[] smoothZoomTowardSelection(int selecXStartIndex,
		...
		// Find the pixel inside the selected area to "zoom toward" such that
		// the
		...
		double targetAspectRatio = (double) numXSelectedIndexes
		...
		double targetZoomFracX = xmap.getOptimalZoomIncrement(
				numXSelectedIndexes, (prevXNumVisible < numXSelectedIndexes));
		double targetZoomFracY = ymap.getOptimalZoomIncrement(
				numYSelectedIndexes, (prevYNumVisible < numYSelectedIndexes));
		...
		// ratio, we don't want to snap to that ratio on the first step - we
		// want
		...
		// We might end up reaching the target aspect ratio before or after
		// we've
		...
		// numXCells, we are at the exact aspect ratio and no correction to it
		// is
		// necessary
		if ((numXCellsShouldHave % 1) == 0
				&& ((int) Math.round(numXCellsShouldHave)) == numXCells) {
			...
			// a possibility that helps to allow aspect ratio changes, but is
			// not
			// relevant here) - force a zoom to happen
			if (zoomXVal == 0 && zoomYVal == 0) {

				// If there's room to zoom 2 in both dimensions
				if (((numXSelectedIndexes <= xmap.getNumVisible() && numXCells > 2) || (numXSelectedIndexes > xmap
						.getNumVisible() && numXCells < (xmap.getMaxIndex() + 2)))
						&& ((numYSelectedIndexes <= ymap.getNumVisible() && numYCells > 2) || (numYSelectedIndexes > ymap
								.getNumVisible() && numYCells < (ymap
								.getMaxIndex() + 2)))) {
					zoomXVal = 2;
					zoomYVal = 2;
				} else if ((numXSelectedIndexes <= xmap.getNumVisible() && numXCells > 1)
						|| (numXSelectedIndexes > xmap.getNumVisible() && numXCells < (xmap
								.getMaxIndex() + 1))) {
					zoomXVal = 1;
				} else if ((numYSelectedIndexes <= ymap.getNumVisible() && numYCells > 1)
						|| (numYSelectedIndexes > ymap.getNumVisible() && numYCells < (ymap
								.getMaxIndex() + 1))) {
					zoomYVal = 1;
				}
			}
		...
		// axis
		else if ((numXCellsShouldHave > numXCells && numYCells > numYSelectedIndexes)
				|| (numXCellsShouldHave < numXCells && numYCells < numYSelectedIndexes)) {
			...
			// To cause the targetAspectRatio to be arrived at gradually, let's
			// only correct by a percentage of the correction:
			numYCellsShouldHave = numYCells + (numYCellsShouldHave - numYCells)
					* aspectRatioFracCorrection;

			// Could check for possible div by zero here, but should be
			// impossible
			targetZoomFracCorrection = Math
					.abs(numYCellsShouldHave - numYCells) / numYCells;
			...
			// //_numYCellsShouldHave_ will be a ratio, i.e. the same as
			// targetZoomFracCorrection
			// //////targetZoomFracCorrection = Math.abs((double) numXCells /
			// numXCellsShouldHave - 1);
			if (numXCells >= numXSelectedIndexes) {
				zoomXVal = xmap.getBestZoomInVal(xPxPos, targetZoomFrac);
			} else {
			...
			// impossible
			targetZoomFracCorrection = Math
					.abs(numXCellsShouldHave - numXCells) / numXCells;
			if (targetZoomFracCorrection > 1.0) {
				targetZoomFracCorrection = Math.abs(numXCellsShouldHave
						- numXCells)
						/ numXCellsShouldHave;
			}

			// This should theoretically be more accurate, but it is not for
			// some
			// reason:
			// targetZoomFracCorrection =
		...
		if (xmap.getNumVisible() == numXSelectedIndexes
			...
			// previously visible corresponding boundary AND the selected area
			// is
			// not surrounding the visible area AND the pixel we are zooming to
			// is on an edge, assume we need to scroll incrementally to bring
			// the
			...
			if ((((// The target start is left of the view
					...
					(selecXStartIndex + numXSelectedIndexes - 1) > (prevXFirstVisible
							+ prevXNumVisible - 1)) &&
					// The view is not inside the target dimensions
					!(selecXStartIndex < prevXFirstVisible && (selecXStartIndex
							+ numXSelectedIndexes - 1) > (prevXFirstVisible
							+ prevXNumVisible - 1))))
					&&
					// The zoom-toward-pixel reported is on an edge
					(xPxPos <= 0 || xPxPos >= (xPxNum - 1))) {

				// Scroll toward by half of the same fraction as the zoom
				// changed
				// We don't want to scroll all the way in 1 sudden jolt, so
				// we'll
				// scroll to be closer to the center of the selection by (half)
				// the same fraction as the incremental zoom (unless we're at
				// the
				// zoom level of the selection)
				double scrollFrac = ((double) xmap.getNumVisible() / (double) numXSelectedIndexes);
				if (scrollFrac > 1) {
				...
				int scrollDistDirX = (selecXStartIndex + (int) Math
						.floor(numXSelectedIndexes / 2)) - centerXIndex;
				...
				// centerXIndex + ".");
				scrollDistDirX = (int) Math.round(scrollDistDirX * scrollFrac);
				 ...
				 * to be done. - Rob
				 */
				// The following correction calculations are to prevent
				// scrolling
				...
				int newNearEdge = (selecXStartIndex + (int) Math
						.floor(numXSelectedIndexes / 2))
						- scrollDistDirX
						- (int) Math.ceil((double) xmap.getNumVisible() / 2);
				int newFarEdge = (selecXStartIndex + (int) Math
						.floor(numXSelectedIndexes / 2))
						- scrollDistDirX
						+ (int) Math.ceil((double) xmap.getNumVisible() / 2);
				...
				if (selecFarEdge <= prevFarEdge && selecFarEdge > newFarEdge) {
					...
					newNearEdge = (selecXStartIndex + (int) Math
							.floor(numXSelectedIndexes / 2))
							- scrollDistDirX
							- (int) Math
									.ceil((double) xmap.getNumVisible() / 2);
					newFarEdge = (selecXStartIndex + (int) Math
							.floor(numXSelectedIndexes / 2))
							- scrollDistDirX
							+ (int) Math
									.ceil((double) xmap.getNumVisible() / 2);
					if (selecFarEdge > newFarEdge
							|| (selecXStartIndex >= prevXFirstVisible && selecXStartIndex < newNearEdge)) {
						scrollDistDirX = 0;
					...
					scrollDistDirX += (newNearEdge - selecXStartIndex)
							+ (int) Math
									.round((1.0 - scrollFrac)
											* ((double) selecXStartIndex - (double) prevXFirstVisible));
					// LogBuffer.println("ScrollX difference after correcting "
					// +
					...
					if (selecXStartIndex < newNearEdge
							|| (selecFarEdge <= prevFarEdge && selecFarEdge > newFarEdge)) {
						scrollDistDirX = 0;
						// LogBuffer.println("ScrollX difference after " +
						// "correcting for far edge overscroll: [" +
						// scrollDistDirX + "].");
				...
				// selecXStartIndex + " + " + numXSelectedIndexes + " / 2) - " +
				// scrollDistDirX);
				xmap.scrollToIndex((selecXStartIndex + (int) Math
						.floor(numXSelectedIndexes / 2)) - scrollDistDirX);
			}
			...
			// the
			// pixel we are zooming to/from into view
			if ((((selecYStartIndex < prevYFirstVisible || (selecYStartIndex
					+ numYSelectedIndexes - 1) > (prevYFirstVisible
					+ prevYNumVisible - 1)) && !(selecYStartIndex < prevYFirstVisible && (selecYStartIndex
					+ numYSelectedIndexes - 1) > (prevYFirstVisible
					+ prevYNumVisible - 1))))
					&& (yPxPos <= 0 || yPxPos >= (yPxNum - 1))) {

				// We don't want to scroll all the way in 1 jolt, so we'll
				// scroll
				// to be closer to the center of the selection by this fraction
				// (unless we're at the zoom level of the selection)
				// Scroll toward by half of the same fraction as the zoom
				// changed
				double scrollFrac = ((double) ymap.getNumVisible() / (double) numYSelectedIndexes);
				if (scrollFrac > 1) {
				...
				int scrollDistDirY = (selecYStartIndex + (int) Math
						.floor(numYSelectedIndexes / 2)) - centerYIndex;
				...
				// centerYIndex + ".");
				scrollDistDirY = (int) Math.round(scrollDistDirY * scrollFrac);
				...
				int newNearEdge = (selecYStartIndex + (int) Math
						.floor(numYSelectedIndexes / 2))
						- scrollDistDirY
						- (int) Math.ceil((double) ymap.getNumVisible() / 2);
				int newFarEdge = (selecYStartIndex + (int) Math
						.floor(numYSelectedIndexes / 2))
						- scrollDistDirY
						+ (int) Math.ceil((double) ymap.getNumVisible() / 2);
				// Make sure we don't scroll an edge from either in view or
				...
				if (selecFarEdge <= prevFarEdge && selecFarEdge > newFarEdge) {
					...
					newNearEdge = (selecYStartIndex + (int) Math
							.floor(numYSelectedIndexes / 2))
							- scrollDistDirY
							- (int) Math
									.ceil((double) ymap.getNumVisible() / 2);
					newFarEdge = (selecYStartIndex + (int) Math
							.floor(numYSelectedIndexes / 2))
							- scrollDistDirY
							+ (int) Math
									.ceil((double) ymap.getNumVisible() / 2);
					if (selecFarEdge > newFarEdge
							|| (selecYStartIndex >= prevYFirstVisible && selecYStartIndex < newNearEdge)) {
						scrollDistDirY = 0;
					...
					scrollDistDirY += (newNearEdge - selecYStartIndex)
							+ (int) Math
									.round((1.0 - scrollFrac)
											* ((double) selecYStartIndex - (double) prevYFirstVisible));
					// LogBuffer.println("ScrollY difference after correcting "
					// +
				...
				ymap.scrollToIndex((selecYStartIndex + (int) Math
						.floor(numYSelectedIndexes / 2.0)) - scrollDistDirY);
			}
		}

		// Update the aspect ratio once we have arrived at our destination (as
		// we
		// are going to be using the starting aspect ratio to use it to
		// gradually
		...
		if (prevXNumVisible > numXSelectedIndexes) {
			...
			redrawPixelBounds[2] = (int) Math
					.round((double) numXSelectedIndexes * pixelsPerXIndex) + 1; // Added
																				// 1
																				// because
																				// sometimes
																				// inaccurate
			...
			redrawPixelBounds[2] = (int) Math.round((double) prevXNumVisible
					* pixelsPerXIndex) + 1; // Added 1 because sometimes
											// inaccurate
		}
		...
		if (prevYNumVisible > numYSelectedIndexes) {
			...
			redrawPixelBounds[3] = (int) Math
					.round((double) numYSelectedIndexes * pixelsPerYIndex) + 1; // Added
																				// 1
																				// because
																				// sometimes
																				// inaccurate
			...
			redrawPixelBounds[3] = (int) Math.round((double) prevYNumVisible
					* pixelsPerYIndex) + 1; // Added 1 because sometimes
											// inaccurate
		}
		...
		if (redrawPixelBounds[0] < 0)
			redrawPixelBounds[0] = 0;
		if (redrawPixelBounds[0] > maxWidth)
			redrawPixelBounds[0] = maxWidth - 1;
		if (redrawPixelBounds[2] < 0)
			redrawPixelBounds[2] = 0;
		if ((redrawPixelBounds[0] + redrawPixelBounds[2]) > maxWidth)
			redrawPixelBounds[2] = maxWidth - redrawPixelBounds[0];

		if (redrawPixelBounds[1] < 0)
			redrawPixelBounds[1] = 0;
		if (redrawPixelBounds[1] > maxHeight)
			redrawPixelBounds[1] = maxHeight - 1;
		if (redrawPixelBounds[3] < 0)
			redrawPixelBounds[3] = 0;
	...
	private void setIndicatorCircleBounds() {
		...
		arrayBoundaryList = findRectangleBoundaries(selectedArrayIndexes, xmap);
		geneBoundaryList = findRectangleBoundaries(selectedGeneIndexes, ymap);

		/*
		 * TODO: Instead of just checking the last(/next) selection position,
		 * should group all small selections together too see if the cluster is
		...
		for (final List<Integer> xBoundaries : arrayBoundaryList) {
			...
			for (final List<Integer> yBoundaries : geneBoundaryList) {
				...
				if (w < 20
						&& h < 20
						&&
						// This is not the first selection and the last
						// selection is far away OR this is the first selection
						// and the next selection either doesn't exists or is
						// far away
						((lastxb >= 0 && Math.abs(xBoundaries.get(0) - lastxb) > 20) || (lastxb < 0 && (arrayBoundaryList
								.size() == 1 || Math.abs(arrayBoundaryList.get(
								1).get(0)
								- xBoundaries.get(1)) > 20)))
						&& ((lastyb >= 0 && Math.abs(yBoundaries.get(0)
								- lastyb) > 20) || (lastyb < 0 && (geneBoundaryList
								.size() == 1 || Math.abs(geneBoundaryList
								.get(1).get(0) - yBoundaries.get(1)) > 20)))) {
					...
					// } else {
					// LogBuffer.println("Selection was too big [" + w + "x" + h
					// + "] or [(" + xBoundaries.get(1) + " - " +
					// xBoundaries.get(0) + ") x (" + yBoundaries.get(1) + " - "
					// + yBoundaries.get(0) + ")].");
				}
