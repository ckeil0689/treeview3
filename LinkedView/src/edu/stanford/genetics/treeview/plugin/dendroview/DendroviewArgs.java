...
public class DendroviewArgs {
	...
	public DendroviewArgs(final String[] args) {
	...
	private int[] parseHeaders(final String optionValue) {
		...
		for (final String number : numbers) {
			final Integer parsed = Integer.parseInt(number);
			if (parsed >= 0) {
				retval.add(parsed);
			}
		}
		final int[] retArray = new int[retval.size()];
	...
	private void parseScaling(final String optionValue)
			throws IllegalOptionValueException {
		if (optionValue == null)
			return;
		final String[] strings = optionValue.split("x");
		if (strings.length != 2)
			throw new jargs.gnu.CmdLineParser.IllegalOptionValueException(
					scaling, optionValue);
	...
	public void printUsage() {
		...
		System.err.println(" -" + filePath.shortForm() + "/--"
				+ filePath.longForm()
