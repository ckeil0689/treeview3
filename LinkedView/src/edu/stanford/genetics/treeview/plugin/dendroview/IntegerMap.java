...
public abstract class IntegerMap implements ConfigNodePersistent {
    ...
    public IntegerMap createMap(String string) {
     ...
     * 
     * This is determined by assuming that the actual index corresponds to 
     * the middle of the block of pixels assigned to that index, 
     * and then linearly interpolating the unit interval onto the block.
     * 
    ...
    public int getPixel (double indval) {
		...
		double residual = indval - base + .5;
		int ibase = (int) base;
		int map = (int) (getPixel(ibase)*(1.0 - residual) + residual 
				* getPixel(ibase + 1));
