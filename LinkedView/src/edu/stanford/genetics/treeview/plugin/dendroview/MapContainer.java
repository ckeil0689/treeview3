...
public class MapContainer extends Observable implements Observer,
	...
	/**
	 * Sets the scale of this MapContainer to the last saved value. The value
	 * needs to be the tile number that is visible on screen. The actual scale
	 * value does not apply if the screen size changed since that would mean
	 * that too few or too many tiles might be visible.
	 */
	...
	/**
	 * This method allows for a more intuitive call from other classes so that
	 * the meaning is conveyed and parameters don't have to be understood and
	 * set first.
	 */
	...
	public void zoomOutCenter(String speed) {
		...
		// LogBuffer.println("zoomOut: Value of firstVisible at start: [" +
		// firstVisible + "].");
		int zoomVal;
		...
		// Ensure that the difference between the previous number visible and
		// the new number visible is even
		if ((numVisible - prevNumVisible) % 2 == 1
				&& numVisible < (getMaxIndex() + 1)) {
			numVisible++;
			tileNumVisible += 1;
		}
		// LogBuffer.println("zoomOut: numVisible has been changed from [" +
		// prevNumVisible + "] to [" + numVisible + "] or more precisely [" +
		// tileNumVisible + "].  Max index: [" + (getMaxIndex() + 1) + "].");
		...
		// LogBuffer.println("zoomOut: firstVisible has been changed from [" +
		// initialFirstVisible + "] to [" + newFirstVisible + "].");
		if (newFirstVisible != firstVisible) {
			scrollToFirstIndex(newFirstVisible);
		}
		// LogBuffer.println("zoomOut: Official new firstVisible: [" +
		// initialFirstVisible + "].");
	...
	/**
	 * Method to zoom in the MapContainer and set the scale for drawing. It's
	 * important to take the range of elements displayed on screen (range of
	 * scrollbar -> visibleAmount()) into account, as well as the amount of
	 * available pixels on screen. Otherwise there will be drawing and selection
	 * issues.
	 *
	...
	// This is for gradually zooming toward the center of the currently
	// displayed dots
	public void zoomInCenter(String speed) {
		...
		// LogBuffer.println("zoomIn: numVisible has been changed from [" +
		// prevNumVisible + "] to [" + numVisible + "].");
		...
		// LogBuffer.println("zoomIn: firstVisible has been changed from [" +
		// firstVisible + "] to [" + newFirstVisible + "].");
		scrollToFirstIndex(newFirstVisible);
	...
	// smoothZoomTowardSelection resides in GlobalView instead of here because
	// it keeps track of the aspect ratio, needing info on both dimensions
	public int zoomToward(int firstIndex, int numIndexes) {
		...
		if (numIndexes < 1 || numIndexes > getAvailablePixels()
				|| firstIndex < 0
				|| (firstIndex + numIndexes - 1) > getMaxIndex()) {
			// LogBuffer.println("zoomToward: Invalid parameters. firstIndex: ["
			// + firstIndex + "] numIndexes: [" + numIndexes +
			// "] available pixels: [" +
		...
		// LogBuffer.println("zoomToward: numVisible has been changed from [" +
		// prevNumVisible + "] to [" + numVisible + "].");
		...
		if (numIndexes == numVisible) {
			...
			// going to be quite centered by the time we get to it,
			// so we'll shift things early on by 1 to correct it so it's
			// centered as soon as possible
			if (diff % 2 == 1 && diff > 2) {
				numVisible--;
				tileNumVisible -= 1;
				numIndexes--;
			...
			// difference in area sizes
				...
				// LogBuffer.println("zoomToward: Zooming in equally. " +
				// newFirstVisible + " = " + firstIndex + " - (int) (" + diff +
				// " / 2)");
			}
		}
		// LogBuffer.println("zoomToward: firstVisible has been changed from ["
		// + initialFirstVisible + "] to [" + newFirstVisible + "].");
		scrollToFirstIndex(newFirstVisible);
	...
	// currently over under the cursor.
	...
	public int zoomTowardPixel(int pixelPos, int customZoomVal) {
		...
		if (customZoomVal >= 0) {
			...
			// LogBuffer.println("zoomTowardPixel: Custom zoom value: [" +
			// zoomVal + "].");
		}
		...
		// Zooming with no smoothing
			...
			// LogBuffer.println("zoomTowardPixel: No smoothing zoom value: [" +
			// zoomVal + "].");
		}
		...
		if (1 == numVisible) {
			...
			// Else if the last visible index is inside the target area
			// (assuming the size of the area we're zooming toward is 1 data
			// cell)
			else if ((initialFirstVisible + prevNumVisible - 1) < firstIndex) {
				newFirstVisible = (firstIndex + 1) - numVisible;
			} else {
				newFirstVisible = (int) ((double) dotOver + 1 - pixelPos
						/ newScale);
				...
				// error)
				if (newFirstVisible > 0
						&& pixelPos / newScale == (int) (pixelPos / newScale)) {
				...
				// the screen
					...
					// +
					// "[" + newFirstVisible + " = (int) ((double) " + dotOver +
					// " + 1 - (double) " + pixelPos + " / " + newScale +
					// ")].");
				}
			}
		}
		...
		if (dotOver != getIndex(pixelPos)) {
			...
			// LogBuffer.println("Warning: The data cell hovered over has shifted. It was over ["
			// + dotOver + "].  Now it is over: [" + getIndex(pixelPos) +
			// "].  Previous dotOver calculation: [firstVisible + (int) ((double) pixelPos / newScale))] = ["
	 ...
	 * have checks early-on to prevent that and return an edge pixel. Make it to
	 * it can return a negative pixel index or a pixel index larger than max. -
	 * Rob
	 */
	...
	public int getZoomTowardPixelOfSelection(int pixelIndexOfSelec,
		...
		// if(pixelIndexOfSelec < 0 && (pixelIndexOfSelec + numPixelsOfSelec) >
		// numTotalPixels) {
		// //I should be able to use the code below for this calculation, but
		// that currently doesn't work and
		// //for right now, I'm debugging other code and need a quick
		// approximate value returned here.
		...
		if (pixelIndexOfSelec < 0
			...
			// LogBuffer.println("Start Pixel: [" + pixelIndexOfSelec +
			// "] and end pixel: [" + pixelIndexOfSelec + " + " + numTotalPixels
			// + "] of selection is out of range/less than 0.");
			// If zooming out, return the furthest pixel, otherwise return the
			// closest pixel
			if (numPixelsOfSelec > numTotalPixels) {
				// LogBuffer.println("Returning Target pixel: [" +
				// (numTotalPixels - 1) + "].");
		...
		// the first pixel so that
			...
			// "] and end pixel: [" + pixelIndexOfSelec + " + " + numTotalPixels
			// +
			// "] of selection is out of range/greater than the number of pixels that are in the display.");
			// If zooming out, return the furthest pixel, otherwise return the
			// closest pixel
			if (numPixelsOfSelec > numTotalPixels) {
				// LogBuffer.println("Returning Target pixel: [0].");
		...
		// The following equation is based on the merging of 2 equations and
		// then solving for targetPixel
		// Let's say that numPixelsOffsetOfSelec is the number of pixels
		// above/left of the start of the selection
		...
		int targetPixel = (int) Math.round((double) numTotalPixels
				* (double) numPixelsOffsetOfSelec
				/ ((double) numPixelsOfSelec - (double) numTotalPixels));
		...
		// negative, we just need to negate it, otherwise,
		// the solution turns out to need a slight adjustment because it is on
		// the outside of the selected area.
		...
		if (targetPixel < 0) {
			...
			// LogBuffer.println("Adjusting Target pixel by offset: [" +
			// targetPixel + "].");
		...
		if (targetPixel < 0 || targetPixel >= numTotalPixels) {
			// LogBuffer.println("Error: target Pixel is out of range: [" +
			// targetPixel + "]");
			targetPixel = 0;
		}
	...
	public int zoomAwayPixel(int pixelPos, int customZoomVal) {
		...
		int prevNumVisible = numVisible;
		numVisible = (int) tileNumVisible;
		// LogBuffer.println("zoomAwayPixel: numVisible has been set to [" +
		// numVisible + "].");
		...
		if (1 == numVisible) {
			...
			// Else if the last visible index is inside the target area
				...
				newFirstVisible = (int) ((double) dotOver + 1 - pixelPos
						/ newScale);
				// newFirstVisible = (int) ((double) dotOverFrac + 1.0 -
				// (double) pixelPos / newScale);
				// LogBuffer.println("zoomAwayPixel: newFirstVisible = dotOver + 1 - pixelPos / newScale.");
				// LogBuffer.println("zoomAwayPixel: " + newFirstVisible + " = "
				// + dotOver + " + 1 - " + pixelPos + " / " + newScale + ".");
		...
		// LogBuffer.println("zoomAwayPixel: Scrolling to new first visible index: ["
		// + newFirstVisible + "].");
		...
		if (correction != 0 && !(newFirstVisible == 0 && correction < 0)
			...
			newFirstVisible += correction;
			LogBuffer
					.println("WARNING: The data cell hovered over has shifted. It was over ["
							+ dotOver
							+ "].  Now it is over: ["
							+ getIndex(pixelPos)
							+ "].  Previous dotOver calculation: [firstVisible + (int) ((double) pixelPos / newScale))] = ["
	...
	public int getBestZoomInVal(int pixel, double targetZoomFrac) {
		...
		// LogBuffer.println("getBestZoomInVal: Called with cells [" + cells +
		// "] targetZoomFrac [" + targetZoomFrac +
		// "] and calculated zoomVal as [" + zoomVal + "].");
		...
		double minDiffThresh = 0.005; // 0=best possible ratio.
										// "Good enough value" for when the
										// ratio of the relative cell position
										// of the cell under
		// the cursor is really close to the ratio of the relative pixel
		...
		// mouse is over
		// double targetFrac = ((double) getIndex(pixel) + 1.0) / ((double)
		// getMaxIndex() + 1.0);
		double targetFrac = (getIndex(pixel) + 1.0 - firstVisible)
				/ (numVisible);
		double diff = 0.0;
		...
		for (i = 0; i <= (zoomMax - zoomMin); i++) {
			...
			// LogBuffer.println("getBestZoomInVal: [relCell = (int) (pixel / numPixels * z)] = ["
			// + relCell + " = (int) (" + pixel + " / " + numPixels + " * " + z
			// + ")].");
			if (z == 0)
				continue;
			...
			if (diff < minDiff) {
				bestZoomVal = z;
				minDiff = diff;
				// Stop if the difference from the target is "good enough", i.e.
				// close to zero
		...
		// bestZoomVal + "] instead of defaults [" + zoomVal + "] Diff: [" +
		// diff + "].");
	 ...
	 * differences that could be handled in an if statement. - Rob
	 */
	// This function performs the reverse of getBestZoomInVal, except it uses
	// the relative pixel position of the cursor to calculate the zoom position
	// that is targeted for smoothing
	public int getBestZoomOutVal(int pixel, double targetZoomFrac) {
		...
		int zoomVal = (int) Math.round(cells / (1 - targetZoomFrac)
				- cells);
		int numPixels = getAvailablePixels();
		...
		// IUf the minimum number of cells to zoom is larger than the current
		// number of cells displayed on this axis - this mitigates occurrences
		// of "zoom jumping"
		// if(zoomMin > cells) {
		// zoomMin = cells - 1;
		// }
		...
		for (i = 0; i <= (zoomMax - zoomMin); i++) {
			...
			// LogBuffer.println("getBestZoomOutVal: [relCell = (int) (pixel / numPixels * z)] = ["
			// + relCell + " = (int) (" + pixel + " / " + numPixels + " * " + z
			// + ")].");
			if (z == 0)
				continue;
	 ...
	 * zoom). Drawing large matrices takes a lot of time, thus animations should
	 * zoom more when there are large abounts of squares to zoom through
	 */
	public double getOptimalZoomIncrement(int targetNumIndexes,
		...
		for (int zf = 1; zf <= maxZF; zf++) {
			...
			// large zoom changes
					a * Math.pow(b, zmfc) < (largerXSize + (zf + 1)
							* ZOOM_INCREMENT * largerXSize)) {
				// if(a*Math.pow(b,zmfc) <= largerXSize) {
				targetZoomFrac = zmfc;
				if (zoomingOut)
					targetZoomFrac += ZOOM_INCREMENT;
				break;
			}
			// targetZoomFrac = zmfc;
		}
		...
		// step, not this one
		if ((zoomingOut && targetZoomFrac * largerXSize > Math.abs(largerXSize
				+ (zoomingOut ? 1 : -1)
	...
	/**
	 * Zooms to the selected index range for this MapContainer. Uses the array-
	 * and geneSelection and currently available pixels on screen retrieved from
	 * the MapContainer objects to calculate a new scale and zoom in on it by
	 * working in conjunction with centerSelection().
	 * 
	 * @param selectedMin
	...
	public void zoomToSelected(final int selectedMin, final int selectedMax) {
		 ...
		 * the edge, so that their selection is centered on the screen, so let's
		 * get the edges of the selection.
		 */
		 ...
		 * calculation (precision/ rounding) errors. Should find out where that
		...
		if (numSelected < minTileNum) {
			...
			else if ((selectedMin + numSelected / 2) > (getMaxIndex() - (minTileNum / 2))) {
	...
	public void centerScrollOnSelection(final int startIndex, final int endIndex) {
		...
		/*
		 * TODO move this outside to avoid multiple calls? center and zoom are
	...
	public void setIndexRange(int i, int j) {
		...
		if (current.getMinIndex() != i || current.getMaxIndex() != j) {
			...
			setupScrollbar();
			/*
			 * TODO improve numVisible implementation Setting default numVisible
			 * here for now '
			 */
			setNumVisible(j + 1);
