...
/**
 * This class allows graphical editing of KaryoColorPresets Also included is a
...
public class KaryoColorPresetEditor extends JPanel implements SettingsPanel {
	 ...
	 */
	public KaryoColorPresetEditor(final KaryoColorPresets up) {
		super();
		presets = up;
	...
	public static void main(final String[] argv) {
		final KaryoColorPresets p = new KaryoColorPresets(new DummyConfigNode(
				"ColorPresets"));
		final KaryoColorPresetEditor e = new KaryoColorPresetEditor(p);
	...
	private class PresetEditPanel extends JPanel {
		...
		public void redoLayout() {
			...
			addS.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(final ActionEvent e) {
					final KaryoColorSet[] toAdd = KaryoColorPresets.defaultColorSets;
		...
		private void removePreset(final int mark) {
			final KaryoColorSet[] tPresetColors = new KaryoColorSet[presetColors.length - 1];
		...
		private void createPreset() {
			final KaryoColorSet toAdd = new KaryoColorSet();
			addPreset(toAdd);
		}

		private void addPreset(final KaryoColorSet toAdd) {
			final KaryoColorSet[] tPresetColors = new KaryoColorSet[presetColors.length + 1];
			final JTextField[] tPresetNames = new JTextField[presetNames.length + 1];
			final JRadioButton[] tDefaultButtons = new JRadioButton[defaultButtons.length + 1];
