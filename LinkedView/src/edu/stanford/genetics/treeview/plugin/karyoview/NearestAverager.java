...
class NearestAverager extends GroupAverager {
	...
	public NearestAverager(final int n) {
		setNum(n);
	}
	...
	@Override
	protected ChromosomeLocus[] getContributors(final ChromosomeLocus locus) {
		final ChromosomeLocus[] ret = new ChromosomeLocus[getNum()];
		...
		// the leftCand != rightCand works for both linear and circular
		// chromosomes,
		// since with a circle they will wrap around, and with a linear they
		// will both
		...
		while ((ncand < num) && (leftCand != rightCand)) {
			...
			if ((leftCand != null) && (rightCand != null)) {
				...
				if (leftDist < rightDist) {
					...
					leftCand = leftCand.getLeft();
					if (leftCand != null)
						leftDist = getDist(leftCand, locus);
				} else {
					// System.out.println("adding right " + rightCand);
					ret[ncand++] = rightCand;
					rightCand = rightCand.getRight();
					if (rightCand != null)
						rightDist = getDist(rightCand, locus);
				}
			}
		}
		return ret;
