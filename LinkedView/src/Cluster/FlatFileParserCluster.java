...
import edu.stanford.genetics.treeview.model.FlatFileStreamLiner;
import edu.stanford.genetics.treeview.model.TestLoad;
/**
* parses a tab-delimited file into a vector of String []. 
* Each String [] represents a row of the file.
*
* This object should be created and configured. 
* None of the real action gets started until the loadIntoTable() 
* routine is called. After loading, the object can be reconfigured 
* and reused to load other files.
*/

public class FlatFileParserCluster {
	...
	public RectDataCluster loadIntoTable() throws LoadException, IOException {
		InputStream stream;
	...
	private RectDataCluster loadIntoTable(InputStream inputStream) 
			throws IOException, LoadException {
		MeteredStream ms = new MeteredStream(inputStream);
		BufferedReader reader = new BufferedReader(new InputStreamReader(ms));
		return FlatFileClusterReader.load(reader, getParseQuotedStrings());
	}
	 ...
	 * Although it's not clear why the ram doesn't get returned to the heap 
	 * when you parse into strings.
	 * 
	 * Note: null entries in number columns will be assigned the value NaN.
	 * 
	 */
	class FlatFileClusterReader {
	...
	private FlatFileClusterReader() {
	}

	public static void main(String[] args) {
		try{
		  	BufferedReader br1 = new BufferedReader(new FileReader(args[0]));
		  	RectDataCluster data1 = FlatFileClusterReader.load(br1, true);
...
//		    Vector data = new FlatFile2RectData().load2(br);
		  	...
		  	for (int i = 0; i < data1.size() && i < 1000; i++){
		  		...
		  		for (int j = 0; j < str1.length; j++){
		  			...
		  			if (FlatFileClusterReader.isDoubleString(str1[j]) && 
		  					FlatFileClusterReader.isDoubleString(str2[j])){
	...
	static public RectDataCluster load(Reader reader, 
			boolean parseQuotedStrings)throws IOException {
		return load(reader, DEFAULT_SEP, 1000, parseQuotedStrings);
	}
		
	static public RectDataCluster load(Reader reader, char sep, 
			boolean parseQuotedStrings)throws IOException {
		return load(reader, sep, DEFAULT_GAPSIZE, DEFAULT_TESTSIZE, 
				parseQuotedStrings);
	}
	static public RectDataCluster load(Reader reader, char sep, int gapSize, 
			boolean parseQuotedStrings)throws IOException {
	 ...
	 * @throws IOException
	 */
	static public RectDataCluster load(Reader br, char sep, int gapSize, 
			int testSize, boolean parseQuotedStrings) throws IOException {
		...
		RectDataCluster data = new RectDataCluster(names, formats, 5000);
	 ...
	 * @return
	 */
	private static ColumnFormat[] makeGeneAnnoString(String[] names,
