 ...
 */
public class ClusterFileMru extends java.util.Observable implements ConfigNodePersistent {
	...
	public boolean getParseQuotedStrings() {
		return (root.getAttribute("quotes", ClusterFileSet.PARSE_QUOTED) == 1);
	}
	public void setParseQuotedStrings(boolean parse) {
		if (parse)
			root.setAttribute("quotes",1, ClusterFileSet.PARSE_QUOTED);
		else
			root.setAttribute("quotes",0, ClusterFileSet.PARSE_QUOTED);
	}
	
	public int getStyle() {
		return root.getAttribute("style", ClusterFileSet.LINKED_STYLE);
	}
	public void setStyle(int style) {
		root.setAttribute("style", style, ClusterFileSet.LINKED_STYLE);
	}
	/**
	 ...
	 */
	public synchronized void removeFileSet(ClusterFileSet fileSet) {
	 ...
	 */
	public synchronized void setLast(ClusterFileSet fileSet) {
	...
	*/
	public synchronized ClusterFileSet addUnique(ClusterFileSet inSet) {
		// check existing file nodes...
		ConfigNode aconfigNode[] = getConfigs();
		for (int i = 0; i < aconfigNode.length; i++) {
			ClusterFileSet fileSet2 = new ClusterFileSet(aconfigNode[i]);
		 ...
		 ClusterFileSet fileSet3 = new ClusterFileSet(configNode);
	...
	public synchronized void removeMoved() {
		...
		for (int i = nodes.length ; i > 0; i--) {
			ClusterFileSet fileSet = new ClusterFileSet(nodes[i-1]);
			if (fileSet.hasMoved()) {
				LogBuffer.println("Could not find " + fileSet.getTxt() + ", " +
						"removing from mru...");
				removeFile(i-1);
				setChanged();
			}
	 ...
	 * @param fileSet
	 */
	public void removeDuplicates(ClusterFileSet inSet) {
		ConfigNode [] nodes = getConfigs();
		int keeper = -1;
		for (int i = nodes.length ; i > 0; i--) {
			ClusterFileSet fileSet = new ClusterFileSet(nodes[i-1]);
			if (fileSet.equals(inSet)) {
				if (keeper != -1) {
					...
					LogBuffer.println("Found duplicate of " + fileSet.getTxt() + ", removing from mru...");
					removeFile(i-1);
					setChanged();
				} else {
					keeper = i;
