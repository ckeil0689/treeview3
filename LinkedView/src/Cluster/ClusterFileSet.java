...
public class ClusterFileSet {
	 ...
	 */
	public ClusterFileSet(ConfigNode configNode) {
		node = configNode;
	}
	 ...
	 */
	public ClusterFileSet(String txt, String dir) {
	 ...
	 */
	public void copyState(ClusterFileSet fileSet) {
	 ...
	 */
	public boolean equals(ClusterFileSet fileSet) {
		return getTxt().equals(fileSet.getTxt());
	}
	...
	/**
	 * @return    The complete path of the txt file
	 */
	...
	/**
		 * @return    The root of the fileset, 
		 * i.e. "test" if the fileset is based on "test.cdt".
	...
	public String getExt() {
		return node.getAttribute("txt", ".txt");
	}
	 ...
	 */
	public void setTxt(String string1) {
		if (string1 != null) {
	...
	public void setExt(String string) {
		node.setAttribute("txt", string, ".txt");
	}
 ...
 // the following concerns types which you can open as...
	...
	public static int getStyleByName(String name) {
 		if (name.equalsIgnoreCase("auto")) {
 			...
 			JOptionPane.showMessageDialog(null, "Error: Invalid Style "+
 					name+". Valid styles " + validStyles);
 			return -1;
 		}
 	}
	/**
	...
	public void notifyMoved() {
		for (FileSetListener listener : fileSetListeners) {
