...
//import treeview.Util;
//import gui.general.PluginManager;

/**
 * Main class of app.LinkedView application. Mostly manages windows, and
 * communication between windows, as well as communication between them.
 *
 * There are two differences between this class and the app.TreeViewApp - which
 * <code>ViewFrame</code> they use. <code>app.LinkedViewApp</code> uses
 * <code>LinkedViewFrame</code>.
 *
 * - app.LinkedViewApp scans for plugins explicitly, app.TreeViewApp doesn't anymore
 *
...
public class LinkedViewApp extends TreeViewApp {
	...
	/** Constructor for the app.LinkedViewApp object */
	// "treeview.plugin.scatterview.ScatterplotFactory"
	// "treeview.plugin.treeanno.GeneAnnoFactory",
	// "treeview.plugin.treeanno.ArrayAnnoFactory"
	// "treeview.plugin.karyoview.KaryoscopeFactory"
		...
		if (sFilePath != null) {
			...
			if (args.getExportType() == null) {
				...
				System.err.println("Must specify file/url to load "
						+ "(using -r) when specifying model.export with -x");
			}
		}
	}
	...
	private boolean attemptExport(final String exportType,
		...
		// mainPanel.model.export(args);
		// return true;
		// }
		// if (exportType.equalsIgnoreCase(tvFrame.getRunning().getName())) {
		// tvFrame.getRunning().model.export(args);
		// return true;
		// }
		// }
