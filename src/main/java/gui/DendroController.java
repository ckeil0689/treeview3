...
import gui.alignment.AtrAligner;
import gui.general.DendroException;
import gui.labels.CopyType;
import gui.labels.LabelContextMenu;
import gui.labels.LabelContextMenuController;
import gui.matrix.ArrayDrawer;
import gui.matrix.ColorExtractor;
import gui.matrix.IntegerMap;
import gui.matrix.MapContainer;
import gui.matrix.MatrixView;
import gui.matrix.MatrixViewController;
import gui.trees.TreeColorer;
import gui.trees.TreePainter;
...
import model.data.matrix.AtrTVModel;
import model.data.matrix.DataModel;
import model.data.matrix.TVModel;
import model.data.trees.ReorderedTreeSelection;
		...
		public void actionPerformed(final ActionEvent e) {
			...
			if(e.getSource() == dendroView.getXYMinusButton()) {
				...
				if((swap_animation_modifiers &&
					...
					dendroView.getInteractiveMatrixView().smoothAnimatedZoomOut();
				}
			}
			else {
...
//			tvModel = new ReorderedDataModel(tvModel, rowIndex, colIndex);
	...
	public void setNewIncluded(final int[] gIncluded, final int[] aIncluded) {
		...
		dendroView.getColLabelView().getLabelSummary().setIncluded(aIncluded);
	}
	...
	private void requestFocusForZoomBtn() {
		...
		if(dendroView.isAFinderBoxFocussed()) { return; }
	...
	public void copyLabels(final CopyType copyType, final boolean isRows) {
		...
		if(!tvFrame.isLoaded() || tvFrame.getRunning() == null) { return; }
