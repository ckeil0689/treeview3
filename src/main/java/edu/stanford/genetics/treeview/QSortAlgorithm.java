 ...
 *
 * 05 Jan 2003: Commented calls to pause(), Added static sort(double) commented
 * refs to stopRequested (camel@alum.mit.edu)
 */

public class QSortAlgorithm {
	void sort(final int a[], final int lo0, final int hi0) throws Exception {
		int lo = lo0;
		int hi = hi0;
		// pause(lo, hi);
		if (lo >= hi)
	...
	public void sort(final int a[]) throws Exception {
		sort(a, 0, a.length - 1);
	}

	static void sort(final double a[], final int lo0, final int hi0) {
		System.out.println("Sorting lo " + lo0 + " hi " + hi0);
		int lo = lo0;
		int hi = hi0;
		// pause(lo, hi);
		if (lo >= hi)
