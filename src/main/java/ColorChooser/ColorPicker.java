...
public class ColorPicker {
	...
	protected void loadActiveColorSetValues() {
		...
		if(!(rg.equalsIgnoreCase(activeColorSet.getColorSchemeName()) || yb
																																				.equalsIgnoreCase(activeColorSet.getColorSchemeName()))) {
	...
	protected ColorSet generateCustomColorSet() {
		...
		return(new ColorSet(name, colorList, fractionList, minVal, maxVal, missing,
												empty));
	}
	...
	/** Updates the missing <code>Color</code> object in the active
	...
	/** Tests if a removal of thumbs can be done. This requires the fractions,
	 * colors and thumb list to be in sync
