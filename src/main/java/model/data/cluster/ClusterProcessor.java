...
public class ClusterProcessor {
	 ...
	 * @param linkMethod - The components.cluster linkage method to be employed
	...
	/**
	 * General components.cluster method that starts a dedicated SwingWorker method which
	 ...
	 * calculations were cancelled by the user, it let's the components.cluster dialog know
	...
	private class Clusterer extends SwingWorker<ClusteredAxisData, Integer> {
		...
		private ClusteredAxisData doHierarchicalCluster() {
			 ...
			 * is the final components.cluster. Initially every row is its own components.cluster
			...
			/* Distance matrix needs to be size 1 when components.cluster finishes! */
			if (distMatrixSize != 1) {
				this.cancel(true);
			}
