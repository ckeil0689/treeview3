...
import model.save.ClusterFileWriter;
...
public class KMeansCluster {
	...
	public void prepare() {
		...
		/* Assign the centroids of each row to their closest seed components.cluster. */
	...
	private int[][] assignToNearestCluster(final double[] meanList) {
		...
		for (int i = 0; i < meanList.length; i++) {
			...
			/* Add the current gene to appropriate components.cluster group */
	...
	/**
	 * Finding new adjusted means for each components.cluster for later reassignment of all
	...
	private void computeNewCentroids(final double[][] clusterMeans) {
		...
		int addIndex = 0;
		// find new mean of each components.cluster
	...
	/* Setters */
	/**
	 * Setter for the seed means for every components.cluster.
	 *
	 * @param clusters
