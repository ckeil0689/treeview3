 ...
 * between rows or columns, using the parameters which the user has chosen.
 *
 * @author CKeil
 *
 */
public class DistMatrixCalculator {
	...
	/*
	 * reference for the matrix to be clustered. Spearman Rank generates the
	 * rank matrix, which needs to be clustered. Without reference variable, the
	 * data matrix would be mutated.
	 */
	 ...
	 * @param distMeasure
	 *            The user choice for the measure of distance to be used.
	 * @param axis
	 *            The axis of the original matrix which should be compared.
	 * @param worker
	 *            The SwingWorker thread which calculates this operation. Used
	 *            to allow cancellation of the process.
	 */
	public DistMatrixCalculator(final double[][] data, final int distMeasure,
			final int axis) {
	...
	/**
	 * This method generates a symmetric distance list based on the Pearson
	 * correlation. Distances of rows on the selected axis will be compared to
	 * each other. The parameters allow for selection of different versions of
	 * the Pearson correlation (absolute Pearson correlation, centered vs.
	 * uncentered).
	 *
	 * @param absolute
	 *            Whether pearson should be absolute or not.
	 * @param centered
	 *            Whether pearson should be centered or not.
	 * @param limit
	 *            Limit of the current data row (working with half-matrix)
	 */
	 ...
	 *            A matrix row.
	 * @param otherRow
	 *            Another matrix row to be compared to the first one.
	 * @param centered
	 *            Whether the correlation should be centered.
	 * @param absolute
	 *            Whether the coefficient will be in absolute terms.
	 * @return The Pearson correlation coefficient.
	 */
	public double calcPearson(final double[] row, final double[] otherRow,
			final boolean centered, final boolean absolute) {
		...
		} else {// works great in components.cluster
			mean_x = 0;
			mean_y = 0;
		}
		...
		/*
		 * Calculate pearson value for current row pair. Stays 0.0 if the
		 * rootProduct is 0
		 */
		...
		double pearson = 0.0;
		if (rootProduct != 0) {
			final double sumOverRoot = sumXY / rootProduct;
	 ...
	 * data object with ranks for the values in the matrix rows. Later on,
	 * pearson correlation will be called on the rewritten data object.
	 */
	public double[] spearman(final double[] row) {
		 ...
		 * rank, which only depends on the number of equal values. The assigned
		 * rank will be the average rank of all these values. If two 1.3s are in
		 * the data, and the first is rank 5, the second rank 6, then their
		 * actual recorded rank will be 5.5.
		 */
		final double[] rankRow = new double[row.length];
		...
		for (int j = 0; j < copyRow.length; j++) {
			...
			if (countDuplicates > 1) {
				final double avgRank = duplicateRankSum
						/ (countDuplicates * 1.0);
	 ...
	 */
	public void taxicab(final boolean isEuclid, final int limit) {
	 ...
	 */
	public double calcEuclid(final double[] row, final double[] otherRow) {
		...
		for (int k = 0; k < row.length; k++) {
			...
			final double diff = row[k] - otherRow[k];
			sum += diff * diff;
		}
	 ...
	 */
	public double calcManhattan(final double[] row, final double[] otherRow) {
	...
	public void setTaskData(final double[][] data) {
