...
public class ThumbBox {
	...
	// How many places a double value, represented by a thumb, is rounded to.
	...
	// defines the area in which thumbs move
	...
	public void dragInnerThumbTo(int inputX) {
		...
		if(selectedThumb instanceof BoundaryThumb) { return; }
	...
	protected void moveSelectedThumbToVal(final double dataVal) {
		...
		if(selectedThumb == null || colorPicker
																						.isOutsideBounds(dataVal)) { return; }

		// Cannot move to boundary fractions
		if(Helper.nearlyEqual(colorPicker.getMinVal(), dataVal) || Helper
																																			.nearlyEqual(colorPicker.getMaxVal(), dataVal)) { return; }
		...
		else if(dataVal < previousData && !Helper
																							.nearlyEqual(previousData, colorPicker.getMinVal())) {
	...
	private int shrinkThumbsToDataRange(double boundaryData, boolean isMin) {
		...
		while(iter.hasNext()) {
			...
			if(thumbs.indexOf(t) == 0 || thumbs.indexOf(t) == thumbs.size() - 1) {
				continue;
			}

			double tData = t.getDataValue();
			if(isMin && (tData < boundaryData || Helper
																									.nearlyEqual(tData, boundaryData))) {
	...
	private void openThumbEditDialog(final Thumb t, int thumbIndex) {
		...
		final EditThumbDialog posInputDialog = new EditThumbDialog(	t, thumbIndex,
																																this,
																																colorPicker.getColorList(),
																																colorPicker.getMean(),
																																colorPicker.getMedian(),
																																colorPicker.getDataCenter(),
																																colorPicker.getDataMin(),
																																colorPicker.getDataMax());
	...
	protected void insertSelectedThumbAt(final double dataVal) {
		...
		if(thumbs.get(selectedIdx) instanceof BoundaryThumb) {
			...
			int insertionIdx = findInsertionIdx(dataVal, thumbs.get(selectedIdx), thumbs);
	 ...
	 * using its corresponding fraction.
	 *
	 * @param thumbIndex - The index of the thumb for which to calculate the value
	 *          in the data range.
	...
	private boolean hasThumbForFraction(final int fracIndex) {
		...
		if(thumbs.size() > fracIndex) {
			double frac = (thumbs.get(fracIndex).getX() - ColorPicker.OFFSET) /
										ColorPicker.WIDTH;
			double frac2 = fractions[fracIndex];

			if(Helper.nearlyEqual(frac, frac2)) { return true; }
		}
	...
	protected boolean askForThumbRemoval(final double dataVal) {
		...
		String warning = "Replace existing handle " +	"for the value " + dataVal +
											"?";
		int choice = JOptionPane.showOptionDialog(JFrame.getFrames()[0], warning, "Replace handle?", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE, null, options, options[0]);
	...
	private void prepareBoundaryThumbSwap(final int outermostInnerIdx,
		...
		removeOldAndInsertNewInnerThumb(outermostInnerIdx, insertionIdx, newInnerThumb);
	}
